...
def handle_text(message):
    ...
    bot.register_next_step_handler(message, choi)
...
def choi(message):
    ...
    bot.register_next_step_handler(message, ge, regular.get_one_regular(message.text.strip()))


def ge(message, regular):
...
def te():
    while True:
        for user in Users.select():
            th = Thread(target=sea, args=(user.id, user.query, user.regular))
            th.start()
            # query, regular = user.query, user.regular
            # links = []
            ...
            #         ]
            #         if items:
            #             bot.send_message(user.id, i, reply_markup=telebot.types.ReplyKeyboardRemove())
            #             bot.send_message(user.id, ", ".join(list(set(items))), reply_markup=telebot.types.ReplyKeyboardRemove())
            #     except Exception:
            #         pass
            # bot.send_message(user.id, "Готово", reply_markup=telebot.types.ReplyKeyboardRemove())
            us = Users.get(Users.id == user.id)
            us.delete_instance()


def sea(idd, query, regular):
    links = []
    for j in search(query, tld="co.in", num=10, stop=10, pause=2): 
        try:
            links.append(j)
