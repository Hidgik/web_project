...
def init_user(id):
    if not Settings.select().where(Settings.id == id):
        setting = Settings(
            id=id, default='Всегда спрашивать', search_sys='Google',
            search_sys_image='Yandex', delim_csv=':', type_file='csv')
...
class SettingResource:
    ...
    def change_object(self, id, obj):
        ...
        elif 'search_sys_image' in obj.keys():
            setting.search_sys_image = obj['search_sys_image']
        setting.save()
